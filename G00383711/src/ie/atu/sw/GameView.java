...
public class GameView extends JPanel implements ActionListener {
    ...
    public GameView(boolean auto) throws Exception {
        ...
        timer.start();
        try {
            String filename = "./resources/game_ai.data";
    ...
    // Add previous move, to save to csv file
    ...
    private void autoMove() {
        if (auto) {
            try {
                ...
                double decision = net.process(gameState, Output.NUMERIC);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        generateNext();
        if (auto && time > 20) {	// generate next if autopilot & time is greater than 20 (2 seconds, to allow plane to enter cave)
            try {
                autoMove();
            } catch (Exception e1) {
                e1.printStackTrace();
        ...
        if (time % 10 == 0) {
            ...
            String gameStateFilePath = "./resources/game_state_data.csv";
            String actionFilePath = "./resources/expected_action_data.csv";
            // Call the writeDataToFile method with both paths, including game state (sample) and previous move (in move method)
    ...
    public double[] sample() {
        var vector = new double[6]; // 6 features
    ...
    // Method to save data to csv files for training
    private void writeDataToFile(double[] gameState, int lastMove, String gameStateFilePath, String actionFilePath) {
        ...
        // Write the last move (which is the expected action for training purposes) to its file
